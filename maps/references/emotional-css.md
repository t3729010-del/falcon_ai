...
## 1. Overview
Primary stylesheet for the Emotional Avatar supporter interface (2226 lines). Provides comprehensive styling for avatar components, voice interactions, chat system, session management, and the humanoid bot display.
...
- **Avatar State Animations**: idle (`idleBreathe`), listening (`listenLean`), thinking (`thinkTilt`), speaking (`speakNod`), preparing/loading (`avatarPulse`), error (red glow). Includes spinning rings (`ringSpin`, `ringSpinCounter` keyframes).
...
- **Voice Visualizer**: Redesigned `.visualizer-section`. In `preparing` mode: fixed position pill shape (`border-radius:50px`, horizontal flex layout, spring-in animation `prepPillIn`). In speaking mode: static position, normal glassmorphism panel. Supports 3 bouncing dots `.typing-indicator` (`typingBounce` keyframe).
...
No direct data persistence. Session state managed via DOM classes: `.active-session`, `.hidden-dropdown`, `.hidden-modal`, `.hidden-avatar`, `.hidden-archive`, `.show-dropdown`. Avatar states applied as class modifiers on `.avatar-container`: `.idle`, `.listening`, `.thinking`, `.speaking`, `.preparing`, `.loading`, `.error`.
